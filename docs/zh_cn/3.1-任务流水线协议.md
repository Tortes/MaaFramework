...
| v5.10 | `Click` 新增 `duration_range` 字段，支持按下与释放之间的随机等待时长 |
| v5.11 | 新增 `RandomDelay` 动作，支持按给定范围随机等待 |
...
- `timeout`: *int*  
    当前**节点**的 `next` 列表识别的超时时间（参见[流程图](#节点流程)），毫秒。可选，默认 20 * 1000（20 秒）。  
    *如果你希望调整当前节点的 `recognition` 识别等待时间，就应该调整上一节点的 `timeout` 属性，而不是调整当前节的。*
...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    ...
    *如果你希望在超出当前节点的 `recognition` 识别等待时间后执行操作，就应该调整上一节点的 `on_error` 属性，而不是调整当前节的。*  
    💡 v5.1 版本起新增支持带属性节点、 NodeAttr 形式对象，或两者结合的异质数组。详情请参考 [节点属性](#节点属性)。
...
- `duration_range`: *array<uint, 2>* **💡 v5.11**  
    随机等待时间范围（毫秒）。可选，默认 `[0, 0]` 。  
  - 框架会在闭区间 `[min, max]` 内随机取一个值并等待对应时长。
  - 若两个值相等，则等价于固定等待时长。
...
- `duration_range`: *array<uint, 2>* **💡 v5.10**  
    点击按下与释放之间的随机等待时间范围（毫秒）。可选，默认 `[50, 50]` 。  
  - 框架会在闭区间 `[min, max]` 内随机取一个值作为本次点击的按压时长。
  - 若两个值相等，则等价于固定按压时长。
