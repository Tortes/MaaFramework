...
| Interception | `512` | 中 | 是 | 否 | 否 | 通过 Interception 驱动注入鼠标事件，适用于常规 Win32 注入不生效的场景 |
...
> - `Interception` 仅实现鼠标链路。若将其同时用于 `keyboard_method`，控制器连接会失败。使用前需要正确安装 Interception 驱动，且通常需要与目标程序相同或更高的权限级别。
