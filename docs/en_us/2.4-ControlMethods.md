...
| Interception | `512` | Medium | Yes | No | No | Injects mouse events through the Interception driver when normal Win32 injection paths do not work |
...
> - `Interception` only implements the mouse path. If it is also selected as `keyboard_method`, controller connection will fail. The Interception driver must be installed correctly, and the process usually needs the same or higher privilege level as the target application.
