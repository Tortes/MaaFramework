...
| v5.10 | Added `duration_range` to `Click` for randomized hold duration between press and release |
| v5.11 | Added `RandomDelay` action for randomized waiting within a configured range |
...
- `timeout`: *int*  
    Timeout for recognition of the current node's `next` list (see [flowchart](#node-flow)) , in milliseconds. Optional, default is 20 * 1000 milliseconds (20 seconds).  
    *If you want to adjust the recognition wait time for the current node, you should adjust the previous node's `timeout` property, not the current node's.*  
...
- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    ...
    *If you want to perform an action when the current node's `recognition` wait time has been exceeded, you should adjust the previous node's `on_error` attribute, not the current node's.*  
    💡 Since v5.1, nodes with attributes, NodeAttr objects, or heterogeneous arrays combining both are supported. See [Node Attributes](#node-attributes) for details.
...
- `duration_range`: *array<uint, 2>* **💡 v5.11**  
    Random wait duration range in milliseconds. Optional, default `[0, 0]`.  
  - The framework samples one value from the closed interval `[min, max]` and waits for that duration.
  - If both values are equal, it behaves as a fixed wait duration.
...
- `duration_range`: *array<uint, 2>* **💡 v5.10**  
    Random wait duration range between press down and release, in milliseconds. Optional, default `[50, 50]`.  
  - The framework samples one value from the closed interval `[min, max]` for each click as the hold duration.
  - If both values are equal, it behaves as a fixed hold duration.
